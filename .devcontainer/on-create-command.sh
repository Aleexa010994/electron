...
if [ ! -f $buildtools/configs/evm.testing.json ]; then
  ...
  # Attempt to auth to RBE. Maintainers have remote execute authority
  # but all users can log in and take advantage of remote cache read.
  if e d rbe login; then
    echo "Authenticated with RBE"
  else
    echo "Failed to authenticate with RBE - disabling remote execution"
    write_config none
  fi
else
